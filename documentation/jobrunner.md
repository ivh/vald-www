# Job Runner: Python Replacement for Shell-Based Job Execution
...
The new system uses Python throughout:
...
job_runner.py: JobRunner.run()
     ...
     ├── Generate config.cfg from database (if VALD_USE_DB_CONFIG=True)
...
### Database-Backed Configs (VALD_USE_DB_CONFIG=True)

When enabled, linelist configurations come from the database:

1. `Linelist` model: Master catalog of all available linelists
2. `Config` model: Configuration sets (system default or user-specific)
3. `ConfigLinelist` model: Links configs to linelists with rank weights
...
### File-Based Configs (VALD_USE_DB_CONFIG=False)

Falls back to reading `.cfg` files from `config/` directory.
...
```python
# Enable Python job runner (default: False for backward compatibility)
VALD_USE_JOB_RUNNER = True

# Enable database configs (default: False)
